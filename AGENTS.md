...
- **This repository owns the workflows under `.github/workflows/`.** They are not generated from an upstream template, so workflow changes are made here directly. This satisfies the `cw-release` non-template workflow guard, which otherwise blocks workflow edits and redirects them to a template/source repository.
- README generation uses the `tronador` CLI, not `make`. The `auto-readme` workflow installs it with `cloudopsworks/install-tronador-cli@v0` and runs `tronador readme build`. Note that `tronador readme build` renders `README.md` only — unlike `make readme`, it does not rebuild `README_DEPS` (`docs/targets.md`), which is committed to this repository. Refresh it with `tronador docs targets` when Makefile targets change.
